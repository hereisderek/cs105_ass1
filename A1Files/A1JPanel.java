...
public class A1JPanel extends JPanel implements MouseListener, KeyListener {
		...
		for (int i = 0; i < cards.length; i++) {
			for (int j = 0; j < cards[i].length; j++) {
				if (cards[i][j] != null && !cards[i][j].getHasBeenRemoved()
						&& cards[i][j].getIsFaceUp()
						&& cards[i][j].pressPointIsInsideCard(pressPt)) {
	...
	private void revealNeighbouringCards(Rectangle removedCardArea,
		...
		case 1:
			...
			break;
		case 3:
			checkRowOfNeighbours(removedCardArea, checkRow = 4,
					rowOfRemovedCard);
			break;
		case 2:
			checkRowOfNeighbours(removedCardArea, checkRow = 1,
					rowOfRemovedCard);
			checkRowOfNeighbours(removedCardArea, checkRow = 3,
					rowOfRemovedCard);
			break;
		}
	}
	...
	private boolean hasNoIntersectingNeighbourInRow(
			Rectangle areaOfCardToCheck, int rowNumberOfRemovedCard) {
		...
		for (int i = 0; i < cards[rowNumberOfRemovedCard].length; i++)
			if (cards[rowNumberOfRemovedCard][i] != null
					&& !cards[rowNumberOfRemovedCard][i].getHasBeenRemoved()
					&& cards[rowNumberOfRemovedCard][i].getCardArea()
							.intersects(areaOfCardToCheck))
				return false;
	...
	private void drawRowOfCards(Graphics g, int whichRow) {
		...
		for (int i = 0; i < cards[whichRow].length; i++) {
			// TODO i dont full understand getComponentPopupMenu, but eclipse
			// did that part for me ^.^
			if (cards[whichRow][i] != null)
				// cards[whichRow][i].drawCard(g, getComponentPopupMenu());
				cards[whichRow][i].drawCard(g, this); // i'll just stick to my
														// own version, although
														// the formal one also
														// works and i dont see
														// the point
	...
	private Card[][] getRandomTableCards(ArrayList<Card> cardStack) {
		...
		Card[][] displayCards = new Card[NUMBER_OF_ROWS][NUMBER_OF_COLS]; // need
																			// to
																			// create
																			// the
																			// 2D
		...
		for (int i = 0; i < displayCards.length; i++) {
			for (int j = 0; j < NON_NULL_CARDS_IN_EACH_ROW[i]; j++) {
				displayCards[i][j] = (cardStack
						.remove(((int) (Math.random() * cardStack.size()))));
	...
	private void writeToFile() {
		...
		try {
			...
			pW.println(userCard.getCardStatusInformation()); // what is this?
	...
	public void loadFromFile() {
		...
		try {
			...
			/*
			 * String a; while ((a = scan.next()) != null) {
			 * System.out.print(" " + a); };
			 */
			this.cardWidth = scan.nextInt();
			this.cardHeight = scan.nextInt();

			userCard.setValue(Integer.parseInt(scan.next()));
			userCard.setSuit(scan.nextInt());
			userCard.setCardArea(scan.nextInt(), scan.nextInt(), 0, 0);
			userCard.setHasBeenRemoved(scan.nextBoolean());
			userCard.setIsFaceUp(scan.nextBoolean());
			// display userCard:
				...
				for (int j = 0; j < NUMBER_OF_COLS; j++) {
					while (scan.hasNextLine()) {
						...
						if (input.equals("null") ){
							...
							card = new Card(lineScan.nextInt(), lineScan.nextInt(),lineScan.nextInt(), lineScan.nextInt(),
									lineScan.nextBoolean(), lineScan.nextBoolean());
							System.out.println(card);
							cards[i][j] = card;
							//System.out.println("Position: " + i + " " + j + " " + card);
							break;
						}
					}
				}
			}
			int times = scan.nextInt(); System.out.println("times: " + times); scan.nextLine();
			cardStack.clear();
			for (int i = 0; i < times/*scan.nextInt()*/; i++){
				input = scan.nextLine();
				Scanner lineScan = new Scanner(input);
				System.out.println(input);
				//System.out.println("lineScan: " + lineScan);
			...
			// } else {
			// System.out.println(input);
			// cards[i][j].setValue(Integer.parseInt(input));
			// cards[i][j].setSuit(scan.nextInt());
			// cards[i][j].setCardArea(scan.nextInt(), scan.nextInt(),
			// i, j);
			// cards[i][j].setHasBeenRemoved(scan.nextBoolean());
			// cards[i][j].setIsFaceUp(scan.nextBoolean());
			// }
			// }
			// }
			//
			...
			// cardStack.add(new Card(scan.nextInt(), scan.nextInt()));
			// scan.nextBoolean();
			// scan.nextBoolean();
			// }
			// // pW.println(userScore);
